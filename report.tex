\documentclass{cslthse-msc}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{breqn}
%\usepackage{makeidx}
\usepackage{graphicx}
\usepackage{framed}
\usepackage{tikz}
\usetikzlibrary{calc, shapes, backgrounds, positioning, matrix}
\usepackage{standalone}
\usepackage[titletoc, header, page]{appendix}
\usepackage{pdflscape}
\usepackage{hyperref}

%\geometry{showframe}

\author{
	Tommy Kvant \\
	{\normalsize \href{mailto:Flavius.Gruian@cs.lth.se}{\texttt{ada09tkv@student.lu.se}}}
}

\title{Task scheduling of multiple industrial robots through constraint programming}

\supervisor{Jacek Malec, \href{mailto:jacek.malek@cs.lth.se}{\texttt{jacek.malek@cs.lth.se}} \& Maj Stenmark, \href{mailto:maj.stenmark@cs.lth.se}{\texttt{maj.stenmark@cs.lth.se}}}
%\supervisor{Maj Stenmark, \href{mailto:maj.stenmark@cs.lth.se}{\texttt{maj.stenmark@cs.lth.se}}}
\examiner{Klas Nilsson, \href{mailto:klas.nilsson@cs.lth.se}{\texttt{klas.nilsson@cs.lth.se}}}

\acknowledgements{

}

\theabstract{

}

\keywords{key word}

\begin{document}
\makefrontmatter
\input{Chapters/introduction}
\input{Chapters/approach}
\input{Chapters/model}
\input{Chapters/assembly}
\input{Chapters/evaluation}
\input{Chapters/discussion}
\input{Chapters/conclusions}

\makebibliography{report}

\begin{appendices}
\input{Appendices/model}
\input{Appendices/assembly_example}
\input{Appendices/tool_manuals}
\end{appendices}
\end{document} 
